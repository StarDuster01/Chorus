...
# Start a new PowerShell window for past hour logs
$oneHourAgo = (Get-Date).AddHours(-1).ToString("yyyy-MM-ddTHH:mm:ssZ")
Start-Process powershell -ArgumentList "-NoExit", "-Command", "kubectl logs -n ragbot -l app=ragbot --since-time='$oneHourAgo'"
